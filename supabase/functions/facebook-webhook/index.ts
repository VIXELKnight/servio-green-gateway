...
async function sendFacebookMessage(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("Facebook send error:", response.status, error);
      return false;
    }
    return true;
  } catch (error) {
...
2. Keep responses concise for Messenger
...
serve(async (req) => {
  ...
  if (req.method === "GET") {
    ...
    if (mode === "subscribe") {
      ...
      // Verify token matches any Facebook channel config
  ...
  if (req.method === "POST") {
    try {
      ...
      console.log("Facebook webhook payload:", JSON.stringify(payload).slice(0, 500));
      ...
      for (const entry of payload.entry) {
        ...
        if (!matchedChannel) {
          console.error("No matching Facebook channel for page_id:", pageId);
          continue;
        }
        ...
        for (const messaging of entry.messaging || []) {
          ...
          console.log(`Processing Facebook message from ${senderId}: ${messageText.slice(0, 100)}`);
          ...
          await sendFacebookMessage(pageId, accessToken, senderId, response);
        }
      }
